...
for file in "$MODELS_DIR"/*.ldr; do
    ...
    if ! "$LDVIEW_BIN" -SaveSnapshot="$OUTPUT_DIR/${filename}_top_down.jpg" -Width=800 -Height=600 -LDrawDir="$LDRAW_DIR" -UseCamera=0 -Latitude=45 -Longitude=45 "$file" > "$LOG_FILE" 2>&1; then
    ...
    if ! "$LDVIEW_BIN" -SaveSnapshot="$OUTPUT_DIR/${filename}_bottom.jpg" -Width=800 -Height=600 -LDrawDir="$LDRAW_DIR" -UseCamera=0 -Latitude=-90 -Longitude=0 "$file" > "$LOG_FILE" 2>&1; then
        echo "  Error: Failed to render bottom image for $filename"
    fi

    # Back image
    echo "  Rendering back image..."
    if ! "$LDVIEW_BIN" -SaveSnapshot="$OUTPUT_DIR/${filename}_back.jpg" -Width=800 -Height=600 -LDrawDir="$LDRAW_DIR" -UseCamera=0 -Latitude=0 -Longitude=180 "$file" > "$LOG_FILE" 2>&1; then
        echo "  Error: Failed to render back image for $filename"
    fi

    # Verify static images
    for suffix in "" "_top" "_front" "_side" "_top_down" "_side_angled" "_bottom" "_back"; do
