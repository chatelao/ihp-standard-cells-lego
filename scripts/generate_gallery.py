...
def load_pdk_urls(filepath):
    pdk_urls = {}
    if not os.path.exists(filepath):
        print(f"Warning: PDK spec file {filepath} not found.")
        return pdk_urls
...
def generate_gallery():
    ...
    pdk_urls = load_pdk_urls(pdk_spec_file)
...
"""
    ...
    for ldr_file in model_files:
        ...
        if name in pdk_urls:
            html_content += f'                    <a href="{pdk_urls[name]}" target="_blank">PDK</a>\n'
        html_content += f'                </div>\n'
        html_content += f'            </div>\n'
        html_content += f'        </div>\n'
    ...
    <footer>
        <p>Standard Cell LEGO Gallery</p>
    </footer>
</body>
</html>
"""
