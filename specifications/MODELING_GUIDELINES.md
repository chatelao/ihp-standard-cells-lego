...
- **Horizontal Scale (X and Z):** 1 LEGO Stud = 0.27 µm.
  - LEF X-coordinate maps to LEGO X-coordinate.
  - LEF Y-coordinate maps to LEGO Z-coordinate.
  - Cell height of 3.78 µm = 14 LEGO studs. We will use **14 studs** as the standard cell height (Z-axis).
...
Based on 1 Stud = 0.27 µm:
- **0.27 µm = 20 LDU**
- **0.0135 µm = 1 LDU**

## 4. Modeling Principles
- We use standard LDraw colors to represent different semiconductor layers.
- The bricks have to fit and interlock as real-world versions. All components must be snapped to the 20 LDU (1 stud) grid to ensure physical buildability.
