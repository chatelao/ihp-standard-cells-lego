...
- **Interconnects (Contacts):** Use 1x1 round bricks (`3062b.dat`) and round plates (`6141.dat`).
...
- **Metal 1:**
  ...
  - `+`/`&`: VDD Rail (Yellow)
  - `-`/`_`: VSS Rail (Black)
- **Connections (Contacts):** Represented by the lowercase version of the layer character (e.g., `i` for input contact, `&` for VDD contact).
